...
export function formatVDOMTree(vnode: any, maxDepth: number = 6, currentDepth: number = 0, showComponentInstance: boolean = true): string {
	...
	if (vnode.attrs) {
		...
		for (const attr of importantAttrs) {
			if (vnode.attrs[attr]) {
				const value = typeof vnode.attrs[attr] === 'string'
					? vnode.attrs[attr]
					: String(vnode.attrs[attr])
