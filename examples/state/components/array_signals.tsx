    ...
    view(vnode: Vnode) {
        return (
            <div style='padding: 20px; border: 1px solid #ccc; margin: 10px;'>
                ...
                <p>
                    Demonstrates accessing signals/stores for array elements using <code>$</code> prefix:
                </p>
                <ul style='font-size: 12px; color: #666; margin-bottom: 15px;'>
                    <li>
                        <code>$s.todos[0]</code> - Access array element value (store or primitive)
                    </li>
                    <li>
                        <code>$s.todos.$0</code> - Access raw signal/store for first element
                    </li>
                    <li>
                        Primitive elements return Signal instances (have <code>.peek()</code>)
                    </li>
                    ...
                    {$s.todos.length > 0 ? (
                        <ul>
                            {$s.todos.map((todo: any, index: number) => {
                                ...
                                const displayValue = isSignal ? elementSignal.peek() : elementSignal // If it's a store, use it directly
                                    ...
                                    <li key={todo.id} style='margin-bottom: 8px;'>
                                        <div style='display: flex; align-items: center; gap: 10px;'>
                                            ...
                                            <span style='font-size: 11px; color: #666;'>
                                                ({isSignal ? 'Signal' : 'Store'} value:{' '}
                                                {typeof displayValue === 'object' ? displayValue?.text : String(displayValue)})
                                            </span>
                                        </div>
                                    </li>
                                )
                ...
                <div style='background: #fff3e0; padding: 15px; border-radius: 4px; font-size: 11px;'>
                    ...
                    <br />
                    <code style='background: white; padding: 2px 4px; border-radius: 2px;'>$s.todos[0].text</code> automatically
                    tracks changes reactively.
                    <br />
                    <code style='background: white; padding: 2px 4px; border-radius: 2px;'>$s.todos.$0</code> returns the store
                    directly (not wrapped in Signal), while primitive elements return Signal instances.
                </div>
            </div>
        )
    }
