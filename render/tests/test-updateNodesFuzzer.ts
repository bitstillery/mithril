...
describe('updateNodes keyed list Fuzzer', () => {
    ...
    [
        ...
        while (tests--) {
            const testCase = fuzzTest(c.delMax, c.movMax, c.insMax)
            test(i++ + ': ' + testCase.list.join() + ' -> ' + testCase.updated.join(), () => {
                render(root, testCase.list.map(x => m(x, {key: x})))
                addSpies(root)
                render(root, testCase.updated.map(x => m(x, {key: x})))

                // For debugging
                if (root.appendChild.mock.calls.length + root.insertBefore.mock.calls.length !== testCase.expected.creations + testCase.expected.moves)
                ...
                expect([].map.call(root.childNodes, n => n.nodeName.toLowerCase())).toEqual(testCase.updated)
				
                // Verify deletions match expected
                expect(root.removeChild.mock.calls.length).toBe(testCase.expected.deletions)
                ...
                expect(root.appendChild.mock.calls.length + root.insertBefore.mock.calls.length)
                    .toBeLessThanOrEqual(testCase.updated.length) // At most, we'd need to reposition every node
            })
        }
    })
})
...
function fuzzTest(delMax, movMax, insMax) {
    ...
    if (movCount > 0) {
        ...
        expected.moves = newPos.filter((pos, i) => {
            ...
            return !itemsInOrder.has(pos) || (i > 0 && pos < newPos[i - 1] && itemsInOrder.has(newPos[i - 1]))
        }).length
    }
