...
describe('domMock', () => {
    ...
    describe('attributes', () => {
        ...
        describe('input[value]', () => {
            ...
            if (typeof Symbol === 'function')
                test('throws when set to a symbol', () => {
                    let threw = false
                    let input = $document.createElement('input')
                    try {
                        input.value = Symbol('')
    ...
    describe('DOMParser for SVG', () => {
        ...
        test('empty document', () => {
            let parser = new $DOMParser()
            let doc = parser.parseFromString('<svg xmlns="http://www.w3.org/2000/svg"></svg>', 'image/svg+xml')
