...
describe('render', () => {
    ...
    test('lifecycle methods work in unkeyed children of recycled keyed', () => {
        ...
        const removeB = spy()
        const a = function () {
            return m('div', {key: 1}, m('div', {oncreate: createA, onupdate: updateA, onremove: removeA}))
        }
        const b = function () {
            return m('div', {key: 2}, m('div', {oncreate: createB, onupdate: updateB, onremove: removeB}))
        }
        render(root, a())
        render(root, b())
        render(root, a())
    ...
    test('does not allow reentrant invocations', () => {
        ...
        render(root, m(A))
        expect(thrown).toEqual(['construct', 'oninit', 'view', 'oncreate'])
        render(root, m(A))
        expect(thrown).toEqual(['construct', 'oninit', 'view', 'oncreate', 'onbeforeupdate', 'view', 'onupdate'])
        render(root, [])
