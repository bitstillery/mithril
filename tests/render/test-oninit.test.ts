...
describe('oninit', () => {
    ...
    test('calls oninit before full DOM creation', () => {
        let called = false
        const vnode = m('div', m('a', {oninit: create}, m('b')))
    ...
    test('No spurious oninit calls in mapped keyed diff when the pool is involved (#1992)', () => {
        ...
        render(root, [m('p', {key: 1, oninit: oninit1}), m('p', {key: 2, oninit: oninit2}), m('p', {key: 3, oninit: oninit3})])
        render(root, [m('p', {key: 1, oninit: oninit1}), m('p', {key: 3, oninit: oninit3})])
        render(root, [m('p', {key: 3, oninit: oninit3})])
